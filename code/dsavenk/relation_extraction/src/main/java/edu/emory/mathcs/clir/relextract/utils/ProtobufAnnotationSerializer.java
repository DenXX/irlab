...
public class ProtobufAnnotationSerializer extends AnnotationSerializer {
    ...
    protected CoreNLPProtos.Sentence.Builder toProtoBuilder(CoreMap sentence, Set<Class<?>> keysToSerialize) {
        ...
        if (sentence.containsKey(NamedEntityTagAnnotation.class)) {
            builder.setNerTag(getAndRegister(sentence, keysToSerialize, NamedEntityTagAnnotation.class));
        }
        if (sentence.containsKey(EntityResolutionAnnotator.EntityResolutionAnnotation.class)) {
            builder.setEntityId(getAndRegister(sentence, keysToSerialize, EntityResolutionAnnotator.EntityResolutionAnnotation.class));
        }
        // Return
        return builder;
    }
    ...
    public CoreNLPProtos.Document toProto(Annotation doc) {
        ...
        if (doc instanceof QuestionAnswerAnnotation) {
            builder.setQuestionLength(((QuestionAnswerAnnotation) doc).getQuestionLength());
            for (Map.Entry<String, String> attr : ((QuestionAnswerAnnotation) doc).getAttributes().entrySet()) {
                builder.addAttribute(CoreNLPProtos.Attribute.newBuilder().setKey(
                        attr.getKey()).setValue(attr.getValue()));
            }
        }
        // Completeness Check
        if (enforceLosslessSerialization && !keysToSerialize.isEmpty()) {
    ...
    protected CoreNLPProtos.Document.Builder toProtoBuilder(Annotation doc, Set<Class<?>> keysToSerialize) {
        ...
        if (doc.containsKey(SpanAnnotator.SpanAnnotation.class)) {
            for (CoreMap span : doc.get(SpanAnnotator.SpanAnnotation.class)) {
                span.remove(TokensAnnotation.class);
                builder.addSpan(toProto(span));
            }
            keysToSerialize.remove(SpanAnnotator.SpanAnnotation.class);
        }
        // Return
        return builder;
    }
    ...
    public CoreNLPProtos.Relation toProto(RelationMention rel) {
        ...
        if (rel.getArgNames() != null) {
            for (String name : rel.getArgNames()) {
                builder.addArgName(name);
            }
        }
        if (rel.getArgs() != null) {
    ...
    protected CoreMap fromProtoNoTokens(CoreNLPProtos.Sentence proto) {
        ...
        if (proto.hasNerTag()) {
            sentence.set(NamedEntityTagAnnotation.class, proto.getNerTag());
        }
        if (proto.hasEntityId()) {
            sentence.set(EntityResolutionAnnotator.EntityResolutionAnnotation.class, proto.getEntityId());
        }
        // Return
        return sentence;
    }
    ...
    public Annotation fromProto(CoreNLPProtos.Document proto) {
        // Set text
        Annotation ann = null;
        if (!proto.hasQuestionLength()) {
            ann = new Annotation(proto.getText());
        } else {
            String text = proto.getText();
            ann = new QuestionAnswerAnnotation(text.substring(0, proto.getQuestionLength()), text.substring(proto.getQuestionLength()));
            for (int i = 0; i < proto.getAttributeCount(); ++i) {
                CoreNLPProtos.Attribute attr = proto.getAttribute(i);
                ((QuestionAnswerAnnotation) ann).addAttribute(attr.getKey(),
                        attr.getValue());
            }
        }
        ...
        // Read spans in a similar way we read sentences.
        if (proto.getSpanCount() > 0) {
            List<CoreMap> spans = new ArrayList<CoreMap>(proto.getSpanCount());
            for (CoreNLPProtos.Sentence span : proto.getSpanList()) {
                CoreMap spanAnnotation = fromProtoNoTokens(span);
                if (!tokens.isEmpty() && span.hasTokenOffsetBegin() &&
                        span.hasTokenOffsetEnd() &&
                        spanAnnotation.get(TokensAnnotation.class) == null) {
                    // Set tokens for sentence
                    int tokenBegin = span.getTokenOffsetBegin();
                    int tokenEnd = span.getTokenOffsetEnd();
                    assert tokenBegin <= tokens.size() && tokenBegin <= tokenEnd;
                    assert tokenEnd <= tokens.size();
                    spanAnnotation.set(TokensAnnotation.class, tokens.subList(tokenBegin, tokenEnd));
                    // Set text
                    int characterBegin = span.getCharacterOffsetBegin();
                    int characterEnd = span.getCharacterOffsetEnd();
                    if (characterEnd <= proto.getText().length()) {
                        // The usual case -- get the text from the document text
                        spanAnnotation.set(TextAnnotation.class, proto.getText().substring(characterBegin, characterEnd));
                    } else {
                        // The document text is wrong -- guess the text from the tokens
                        spanAnnotation.set(TextAnnotation.class, recoverOriginalText(tokens.subList(tokenBegin, tokenEnd), span));
                    }
                }
                spans.add(spanAnnotation);
            }
            ann.set(SpanAnnotator.SpanAnnotation.class, spans);
        }
