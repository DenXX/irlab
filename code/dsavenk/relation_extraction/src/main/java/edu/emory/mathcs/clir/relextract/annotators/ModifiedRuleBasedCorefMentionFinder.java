...
public class ModifiedRuleBasedCorefMentionFinder implements CorefMentionFinder {
    ...
    public ModifiedRuleBasedCorefMentionFinder(boolean allowReparsing) {
    ...
    public List<List<Mention>> filterPredictedMentions(List<List<Mention>> allGoldMentions, Annotation doc, Dictionaries dict){
        ...
        for(int i = 0 ; i < allGoldMentions.size(); i++){
            ...
            // MODIFICATION: following the todo
...
//    this.maxID = _maxID;
    ...
    protected static void removeSpuriousMentions(CoreMap s, List<Mention> mentions, Dictionaries dict) {
        ...
        for(Mention m : mentions){
            ...
            // bareNPRule
            // MODIFICATION: I do want these NPs
            ...
            // MODIFICATION: I do want these, useful for nationalities, etc.
...
//                    }
    ...
    private static boolean inStopList(Mention m) {
        ...
        if (mentionSpan.equals("here") || mentionSpan.equals("there")
                || mentionSpan.equals("ltd.")
                || mentionSpan.equals("some")
                || mentionSpan.equals("less")
                || mentionSpan.equals("more")
                || mentionSpan.equals("a lot")) return true;

        if (mentionSpan.startsWith("'s ")) return true;

        if (mentionSpan.startsWith("etc.") ||
                mentionSpan.endsWith("etc.")) return true;
