package edu.emory.mathcs.ir.kb;
...
//                model_.getResource(convertFreebaseMidRdf("type.datetime")));
    ...
    public static synchronized KnowledgeBase getInstance(
            String modelLocation, String indexLocation) {
        if (kb_ == null) {
            kb_ = new KnowledgeBase(modelLocation, indexLocation);
        }
    ...
    public long getTripleCount(String mid) {
        if (dictSearcher_ != null) {
            ...
            try {
                ...
                if (docs.totalHits > 0)
                    return Long.parseLong(dictSearcher_.doc(docs.scoreDocs[0].doc).get(TRIPLE_COUNT_FIELD));
                else return 0L;
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    public Pair<String, String> getPredicateDomainAndRange(String predicate) {
        if (domainRangeCache_.containsKey(predicate))
            return domainRangeCache_.get(predicate);
        ...
        if (iter.hasNext())
            res.first = iter.nextStatement().getObject().asResource().getURI();
        ...
        if (iter.hasNext())
            res.second = iter.nextStatement().getObject().asResource().getURI();
     ...
     * @return A set of
     * {@link edu.emory.mathcs.ir.kb.KnowledgeBase.Triple}
     * objects.
     */
    ...
    public Set<String> lookupEntitiesByName(String name) {
        if (nameLookupCache_.containsKey(name))
            return nameLookupCache_.get(name);
